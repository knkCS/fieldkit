...
│   ├── virtual-table-row-spec.ts # ADR-0017's rules as pure functions: which of the two ways a Virtual Table declares its Row Spec (linked, embedded, both, neither), the Blueprint a linked one names, and the flat value types a column may be — shared by validateSpec, resolveSpec and the renderer
