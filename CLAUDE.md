...
- **Composable Zod**: Each plugin provides `toZodType()`. `specToZodSchema()` composes them. Consumers can override. A container type gets an optional second argument that composes its own children (`ComposeChildSchema`, and `ComposeChildDefaults` for `defaultValue`) — that is how a Fieldset validates and seeds what it holds without the shared builder knowing its name.
