...
import type { FieldTypePlugin, CellProps } from "../../schema/plugin";
...
describe("getCellForFieldType", () => {
  it("should generate columns from schema", () => {
    const schema: Schema = [
      ...
      makeField({
        field_type: "number",
        config: { name: "Count", api_accessor: "count", required: false, instructions: "" },
      }),
    ];
  ...
  it("should skip section fields", () => {
    const schema: Schema = [
      ...
      makeField({
        field_type: "section",
        config: { name: "Details", api_accessor: "details_section", required: false, instructions: "" },
      }),
    ];
  ...
  it("should skip hidden fields", () => {
    const schema: Schema = [
      ...
      makeField({
        field_type: "text",
        config: { name: "Secret", api_accessor: "secret", required: false, instructions: "", hidden: true },
      }),
    ];
  ...
  it("should use plugin cellComponent when available", () => {
    ...
    render(
      <TableRenderer schema={schema} data={[{ title: "Hello" }]} />,
    );
  ...
  it("should fall back to string rendering when no cellComponent", () => {
    ...
    render(
      <TableRenderer schema={schema} data={[{ count: 42 }]} />,
    );
