...
export function GroupField({ field, readOnly }: FieldProps<GroupSettings>) {
  ...
  const canAdd = settings.max_items === undefined || items.length < settings.max_items;
  const canRemove = settings.min_items === undefined || items.length > settings.min_items;
      ...
      {items.map((item, index) => (
        ...
        >
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "0.5rem" }}>
            <span style={{ fontSize: "0.875rem", color: "#666" }}>Item {index + 1}</span>
            {!readOnly && canRemove && (
              <button
                type="button"
