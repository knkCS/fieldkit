      ...
      {showTypePicker && (
        ...
        >
          ...
          {allowedBlocks.length === 0 && (
            <span style={{ fontSize: "0.875rem", color: "#999" }}>No block types configured</span>
          )}
        </div>
      )}
      {items.map((item, index) => {
        const blockType = (item as Record<string, unknown>)._type as string | undefined;
                ...
                <div style={{ display: "flex", gap: "0.25rem" }}>
                  {index > 0 && (
                    <button
                      ...
                      style={{ padding: "0.125rem 0.5rem", border: "1px solid #ccc", borderRadius: "4px", background: "white", cursor: "pointer", fontSize: "0.75rem" }}
                    >
                      Up
                    </button>
                  )}
