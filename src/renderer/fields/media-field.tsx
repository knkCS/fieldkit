  ...
  if (!mediaAdapter) {
    return (
      <div style={{ marginBottom: "1rem" }}>
        ...
        <p style={{ color: "#999", fontSize: "0.875rem" }}>Media adapter not configured</p>
      </div>
    );
  }
    ...
    <div style={{ marginBottom: "1rem" }}>
      ...
      <Controller
        ...
        render={({ field: formField, fieldState }) => {
          const currentIds: string[] = Array.isArray(formField.value) ? formField.value : [];

          const getMediaItem = (id: string) => resolvedItems.find((item) => item.id === id);
          ...
          const atLimit = settings.max_items != null && currentIds.length >= settings.max_items;
