...
- Child fields are not validated in Zod — items use `z.record(z.unknown())` instead of composing child field schemas. The plugin contract no longer stands in the way: `toZodType` takes an optional composer for child fields (#53, used by Fieldset), so a row could compose its children. Doing it here is a separate change with its own blast radius — every existing Group's stored rows would start being checked.
