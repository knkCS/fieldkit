...
export function CheckboxesField({ field, readOnly }: FieldProps<CheckboxesSettings>) {
  ...
  return (
    <div style={{ marginBottom: "1rem" }}>
      <fieldset style={{ border: "none", padding: 0, margin: 0 }}>
        <legend style={{ display: "block", marginBottom: "0.25rem", fontWeight: 500 }}>
          {field.config.name}
              ...
              <>
                <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}>
                  {Object.entries(options).map(([value, label]) => (
                    <label key={value} style={{ display: "flex", alignItems: "center", gap: "0.5rem", cursor: readOnly ? "default" : "pointer" }}>
                      <input
                        type="checkbox"
                        value={value}
                        ...
                        onChange={(e) => {
                          if (e.target.checked) {
                            ...
                            formField.onChange(selected.filter((v) => v !== value));
                          }
                        }}
                        disabled={readOnly}
                      />
