...
export function VirtualTableField({ field, readOnly }: FieldProps<VirtualTableSettings>) {
  ...
  if (!blueprintAdapter) {
    return (
      <div style={{ marginBottom: "1rem" }}>
        ...
        <p style={{ color: "#999", fontSize: "0.875rem" }}>Blueprint adapter not configured</p>
      </div>
    );
  }
    ...
    <div style={{ marginBottom: "1rem" }}>
      ...
      <Controller
        ...
        render={({ field: formField, fieldState }) => {
          const rows: Record<string, unknown>[] = Array.isArray(formField.value) ? formField.value : [];

          if (loading) {
            return <p style={{ color: "#999", fontSize: "0.875rem" }}>Loading table schema...</p>;
          }
