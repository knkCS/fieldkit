...
describe("SpecEditor", () => {
	it("renders field list with names", () => {
		...
		render(<SpecEditor schema={schema} onChange={vi.fn()} plugins={plugins} />);
	...
	it("has add button", () => {
		render(<SpecEditor schema={[]} onChange={vi.fn()} plugins={plugins} />);
	...
	it("calls onChange without the removed field when remove is clicked", () => {
		...
		render(
			<SpecEditor schema={schema} onChange={onChange} plugins={plugins} />,
		);
