...
function FieldModalInner({
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any -- settings are plugin-specific
  ...
  useEffect(() => {
    ...
    if (field) {
      ...
      setDefaultValue(field.config.default_value != null ? String(field.config.default_value) : "");
      setHidden(field.config.hidden ?? false);
      setReadOnly(field.config.read_only ?? false);

      setMinLength(field.validation?.min_length != null ? String(field.validation.min_length) : "");
      setMaxLength(field.validation?.max_length != null ? String(field.validation.max_length) : "");
  ...
  const canSave = name.trim() !== "" && apiAccessor.trim() !== "" && !accessorError;
    ...
    >
      ...
      >
        ...
        <div style={fieldGroupStyle}>
          ...
          {accessorError && (
            <span data-testid="accessor-error" style={{ color: "#e53e3e", fontSize: "12px" }}>
              {accessorError}
            </span>
          )}
        </div>
        ...
        {SettingsComponent ? (
          ...
          <p style={{ color: "#888", fontSize: "14px" }}>No additional settings</p>
        )}
