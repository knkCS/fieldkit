...
describe("virtualTablePlugin", () => {
  ...
  it("should generate array of record objects Zod type", () => {
    const field: Field<VirtualTableSettings> = {
      ...
      config: { name: "Entries", api_accessor: "entries", required: false, instructions: "" },
    ...
    expect(zodType.safeParse([{ title: "Hello", count: 42 }]).success).toBe(true);
  ...
  it("should have default settings with max_records_per_page", () => {
    expect(virtualTablePlugin.defaultSettings).toEqual({ max_records_per_page: 25 });
  });
});
