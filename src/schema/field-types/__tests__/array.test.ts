...
describe("arrayPlugin", () => {
  ...
  it("should generate dynamic mode Zod type (array of key-value objects)", () => {
    const field: Field<ArraySettings> = {
      field_type: "array",
      config: { name: "Metadata", api_accessor: "metadata", required: false, instructions: "" },
    ...
    expect(zodType.safeParse([{ key: "color", value: "red" }]).success).toBe(true);
    expect(zodType.safeParse([{ key: "color", value: "red" }, { key: "size", value: "large" }]).success).toBe(true);
  ...
  it("should generate keyed mode Zod type (record of strings)", () => {
    const field: Field<ArraySettings> = {
      field_type: "array",
      config: { name: "Settings", api_accessor: "settings", required: false, instructions: "" },
    ...
    const zodType = arrayPlugin.toZodType(field);
    expect(zodType.safeParse({}).success).toBe(true);
    expect(zodType.safeParse({ color: "red", size: "large" }).success).toBe(true);
