...
None outstanding — the last of them, `locked_settings`, landed on 2026-08-04.

**Landed since this was written:** `single_reference` (ADR-0008), which holds `Reference | null` and which core has no counterpart for; `toc_reference`'s removal (ADR-0010), which put shared at 21 and core-only at 6 exactly as predicted; and **`locked_settings`** (ADR-0011) on `FieldConfig` — the config panel disables each frozen control and shows the Consumer's reason beside it, and the write is dropped even when a settings component never checks the list. Core still has to populate it before `pin_mode` is frozen anywhere real.
